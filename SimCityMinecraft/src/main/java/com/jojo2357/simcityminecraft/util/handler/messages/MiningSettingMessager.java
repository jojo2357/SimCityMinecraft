...
public class MiningSettingMessager{
	private int index;
	private BlockPos pos;
	
	public MiningSettingMessager(int index) {
		this.index = index;
	}
	
	public MiningSettingMessager(int index, BlockPos pos) {
		this.index = index;
		this.pos = pos;
	}
    ...
    public static MiningSettingMessager deserialize(PacketBuffer buffer) {
        int index = buffer.readInt();
        BlockPos pos = buffer.readBlockPos();
        return new MiningSettingMessager(index, pos);
    }
    
    public static boolean handle(MiningSettingMessager message, Supplier<NetworkEvent.Context> contextSupplier) {
        ...
        if (context.getDirection().getReceptionSide() == LogicalSide.SERVER) {
            context.enqueueWork(() -> {
                ...
                if (entity instanceof SimMineBlockTileEntity) {
                	((SimMineBlockTileEntity)entity).settingChanged(message.index);
                }
            });
        }
