...
public class SimFarmBlockBlock extends ContainerBlock {
   public static final DirectionProperty FACING = BlockStateProperties.HORIZONTAL_FACING;
   public static final IntegerProperty COLORSTATE = IntegerProperty.create("colorstate", 1, 3);

   public SimFarmBlockBlock(Block.Properties properties) {
      super(properties);
   ...
   public BlockState getStateForPlacement(BlockItemUseContext context) {
      ...
      return this.getDefaultState().with(FACING, context.getPlacementHorizontalFacing().getOpposite()).with(COLORSTATE, 1);
   }

   public TileEntity createNewTileEntity(IBlockReader worldIn) {
      return new SimFarmBlockTileEntity();
   }
   ...
   public void onBlockPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
      if (stack.hasDisplayName()) {
         ...
         if (tileentity instanceof SimFarmBlockTileEntity) {
            ((SimFarmBlockTileEntity)tileentity).setCustomName(stack.getDisplayName());
         }
      }
   ...
   public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
      if (worldIn.isRemote) {
         ...
         TileEntity tileentity = worldIn.getTileEntity(pos);
         if (tileentity instanceof SimFarmBlockTileEntity) {
        	 NetworkHooks.openGui((ServerPlayerEntity) player, (SimFarmBlockTileEntity) tileentity, pos);
